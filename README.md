...
- the built-in `powersave` profile now uses a slightly higher gain floor, a somewhat stronger recovery step, and a slower / less eager gradient trigger so it stays conservative without remaining quite as sticky at the floor as before
...
- the current built-in powersave tuning also uses a longer control interval and a wider gradient margin so short pressure spikes do not retune the controller too eagerly
