...
- the built-in `powersave` profile now also allows a larger delay budget, keeps a higher gain floor, and uses a gentler backoff curve so it stays conservative without remaining quite as sticky at the floor as before
...
- the current built-in powersave tuning also tolerates more queue delay and uses a softer backoff curve, so it is less likely to get pinned near the minimum gain floor under sustained pressure
