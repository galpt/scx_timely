...
- the built-in `powersave` profile now also keeps a higher gain floor, but uses a more moderate delay budget and backoff curve so it stays conservative without becoming quite as slow and sticky under sustained pressure
...
- the current built-in powersave tuning now aims for a middle ground: less floor-sticky than the earlier preset, but also less permissive about sustained queue growth than the previous powersave pass
